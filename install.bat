...
powershell -ExecutionPolicy Bypass -NonInteractive -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut([Environment]::GetFolderPath('Desktop')+'\Libiry.lnk');$s.TargetPath='%~dp0venv\Scripts\pythonw.exe';$s.Arguments='main.py';$s.WorkingDirectory='%~dp0';$s.IconLocation='%~dp0resources\icons\Libiry.ico';$s.Save()"
if errorlevel 1 (
